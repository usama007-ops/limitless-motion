...
export default function EditFastingBreakfast() {
  ...
  const fetchData = useCallback(async () => {
    try { const data = await getFastingBreakfasts(); const found = (data || []).find((v) => v.id === params.id); if (!found) throw new Error('Not found'); setItem(found) }
    catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }, [params.id])

  useEffect(() => { document.title = 'Edit Breakfast - Admin - Limitless Motion'; fetchData() }, [fetchData])
  ...
  async function handleSubmit(data) {
    ...
    try {
      ...
      await adminUpdate('fasting_breakfasts', params.id, payload); setSuccess(true)
    } catch (err) { setError(err.message) }
    finally { setSaving(false) }
  }

  if (loading) return <div><AdminPageHeader title="Edit Breakfast" /><div className="animate-pulse bg-card border border-border rounded-lg p-6 h-64" /></div>
  if (error && !item) return <div><AdminPageHeader title="Edit Breakfast" /><div className="bg-destructive/10 border border-destructive/30 rounded-lg p-6 text-destructive">{error}</div></div>
