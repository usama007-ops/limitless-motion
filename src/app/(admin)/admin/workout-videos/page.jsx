...
export default function AdminWorkoutVideos() {
  ...
  async function handleNewSubmit(e) {
    e.preventDefault()
    const form = e.target
    const data = { title: form.title.value, difficulty: form.difficulty.value, category: form.category.value, description: form.description.value }
    if (form.duration.value) data.duration = Number(form.duration.value)
    if (form.video_file_url.value) data.video_file_url = form.video_file_url.value
    setSaving(true)
    try {
        ...
        <div className="bg-card border border-border rounded-lg p-6 mb-6 max-w-2xl">
          ...
          <form onSubmit={handleNewSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="block text-sm font-medium text-foreground mb-1">Video URL</label><input name="video_file_url" placeholder="https://..." className="w-full px-3 py-2 text-sm bg-background border border-border rounded-md focus:outline-none focus:ring-2 focus:ring-primary/30 text-foreground" /></div>
            </div>
