...
export default function MovementBuilderCanvas({ open, onClose }) {
  ...
  useEffect(() => {
    if (!selectedProgram || !open) return
    loadData(selectedProgram)
  }, [selectedProgram, open])
    ...
    try {
      for (let d = 0; d < 7; d++) {
        const { data } = await adminCreate('workout_days', {
          program_id: selectedProgram, week_number: nextWeek, day_of_week: d + 1,
          day_name: `Week ${nextWeek} - Day ${d + 1}: ${names[d]}`, focus_area: names[d], estimated_duration: 40,
        })
        for (let e = 0; e < 2; e++) {
          await adminCreate('exercises', { program_id: selectedProgram, day_id: data[0].id, exercise_name: '', sets: 3, reps: '', muscle_groups: '', form_tips: '' })
        }
      }
  ...
  async function handleAddExercise(dayId) {
    try {
      await adminCreate('exercises', { program_id: selectedProgram, day_id: dayId, exercise_name: '', sets: 3, reps: '', muscle_groups: '', form_tips: '' })
      await loadData(selectedProgram)
    } catch (e) { alert('Failed: ' + e.message) }
  }
  ...
  async function handleRemoveExercise(exId) {
    ...
    try { await adminDelete('exercises', exId); await loadData(selectedProgram) }
    catch (e) { alert('Failed: ' + e.message) }
  }
